-- Cloud sync for AnkiKOAi via KOReader's SyncService.
...
-- Only treat missing remote keys as intentional deletes when the download
-- looks complete (avoids mass data loss from truncated cloud snapshots).
local function remote_deletes_trusted(income_cards, cached_cards, income_idx, cached_idx)
    if #income_cards == 0 or #cached_cards == 0 then
        return false
    end
    local matched = 0
    for key, _ in pairs(cached_idx) do
        if income_idx[key] then
            matched = matched + 1
        end
    end
    return matched >= math.floor(#cached_cards * 0.8)
end
...
function CardSync.merge_cards(local_path, cached_path, income_path)
    ...
    local trust_remote_deletes = remote_deletes_trusted(
        income_cards, cached_cards, income_idx, cached_idx)
    ...
    for _i, lc in ipairs(local_cards) do
        ...
        if not seen[key] then
            ...
            elseif cc and #income_cards > 0 and trust_remote_deletes then
                -- Card in local + cached but NOT in remote → deleted remotely.
                -- Drop it (don't add to merged). But only if income is non-empty
                -- (empty income = first sync, not a mass delete) and the remote
                -- snapshot looks complete (not a truncated download).
            else
